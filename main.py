...
class SmartMiner(wx. Frame):
    ...
    def __init__(self, *args, **kw):
        ...
        self.command = wx.TextCtrl(self.pnl, id=-1,value=self.config['command'], pos=(10, 30), size=(600, 25))
        ...
        # pool pool
        self.poolTxt = wx.StaticText(self.pnl, label="礦池地址", pos=(10,80))
        self.pool = wx.TextCtrl(self.pnl, id = -1,value=self.config['pool'], pos=(80, 75),size=(310, 25))
        ...
        # pool wallet
        self.walletTxt = wx.StaticText(self.pnl, label="錢包地址", pos=(10, 120))
        self.wallet = wx.TextCtrl(self.pnl, id = -1,value=self.config['ewal'] , pos=(80, 115),size=(310, 25))
        ...
        # pool eworker
        self.workerTxt = wx.StaticText(self.pnl, label="礦工名稱", pos=(10, 160))
        self.worker = wx.TextCtrl(self.pnl, id = -1,value=self.config['eworker'], pos=(80, 155),size=(310, 25))
        ...
        # pool email
        self.emailTxt = wx.StaticText(self.pnl, label="Email", pos=(10, 200))
        self.email = wx.TextCtrl(self.pnl, id = -1,value=self.config['email'], pos=(80, 195),size=(310, 25))
        ...
        # tune tyoe
        self.timer1 = wx.RadioButton(self.pnl, label='契約用時間', pos=(400, 105))
        self.timer2 = wx.RadioButton(self.pnl, label='住商簡易型時間', pos=(400, 130))

        self.timer1.Bind(wx.EVT_RADIOBUTTON, self.onChecked)
        self.timer2.Bind(wx.EVT_RADIOBUTTON, self.onChecked)

        # help author checkBox
        self.helpAuthorCheckBox = wx.CheckBox(self.pnl, -1, "也幫作者挖10分鐘(感恩)", pos=[400, 150] ,size=[300, 30])
        ...
        # version
        self.versionTxt = wx.StaticText(self.pnl, label="時間礦工"+self.version+" (Claymore "+self.versionClaymore+")", pos=(10,230))
    ...
    def onTimer(self, event):
        ...
        self.versionTxt = wx.StaticText(self.pnl, label=time, pos=(400, 72))
        font = self.versionTxt.GetFont()
        font.PointSize += 5
        self.versionTxt.SetFont(font)
